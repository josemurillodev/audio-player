...
class Id3DataView extends DataView<ArrayBuffer> {
  getString(start: number, length = 1) {
    let result = '';
...
const getAPIC = (a: DataView, id: number, x: number, size: number) => {
  try {
    const encodingByte = a.getUint8(id + x);
    ...
    const mimeBytes = new Uint8Array(a.buffer.slice(offset, mimeEnd));
    const mimeType = asciiDecoder.decode(mimeBytes);
    offset = mimeEnd + 1;

    // 2. Read picture type (1 byte)
    const pictureType = a.getUint8(offset);
    offset += 1;
    ...
    if (encodingByte === 1 || encodingByte === 2) {
      ...
      while (
        descEnd + 1 < id + size &&
        !(a.getUint8(descEnd) === 0 && a.getUint8(descEnd + 1) === 0)
      ) {
        descEnd += 2;
      }
    } else {
      // Single-byte encoding: null is one byte
      while (descEnd < id + size && a.getUint8(descEnd) !== 0) {
        descEnd += 1;
      }
    }

    const descBytes = new Uint8Array(a.buffer.slice(offset, descEnd));
    ...
    const imageData = new Uint8Array(a.buffer.slice(offset, id + x + size));
...
function decodeSynchsafeInt(view: DataView, offset: number): number {
  return (
...
export const readID3 = (buffer: ArrayBuffer) => {
  ...
  for (; true; ) {
    ...
    frameSize = isV24
      ? decodeSynchsafeInt(dataView, id3FrameSize + 4)
    ...
    if (frameName !== 'APIC' || (frameName === 'APIC' && !hasApic)) {
      ...
      tags[frameName] = hasApic
        ? getAPIC(dataView, id3FrameSize, nameSize, frameSize)
